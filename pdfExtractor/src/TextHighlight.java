 ...
 */
public class TextHighlight extends PDFStreamEngine {

    private static final String thisClassName = PDFTextStripper.class.getSimpleName().toLowerCase();

    public static List<String> searchedWords = null;
    public String outputFileName = null;
    public static int ignoreStartingPages = -1;
     ...
     */
    public TextHighlight() throws IOException {
        super(ResourceLoader.loadProperties(
                "TextCoordinates.properties", true));
     ...
     */
    public TextHighlight(Properties props) throws IOException {
        super(props);
        this.outputEncoding = null;
        normalize = new TextNormalize(this.outputEncoding);
    }
     ...
     * @throws IOException If there is an error reading the properties.
     */
    public TextHighlight(String encoding) throws IOException {
        super(ResourceLoader.loadProperties(
                "TextCoordinates.properties", true));
    ...
    protected void processPages(List<COSObjectable> pages) throws IOException {
        ...
        if (ignoreStartingPages > 0) {
            for (int i = 0; i < ignoreStartingPages; i++) {
                pageIter.next();
                currentPageNo++;
            }
        }
        while (pageIter.hasNext()) {
            ...
            System.out.println("Page no " + currentPageNo);
            currentPageNo++;
            if (contentStream != null) {
                //System.out.println("content Stream not null "+currentPageNo);
                COSStream contents = contentStream.getStream();
                processPage(nextPage, contents);
            }
        }
        System.out.println("coming out of the loop gracefully " + currentPageNo + " " + pageIter);
    ...
    protected void writePage() throws IOException {
        ...
        List<String> matchTo = new ArrayList<String>();
        for (int i = 0; i < searchedWords.size(); i++) {
            String match = searchedWords.get(i).replace(" ", "");
        ...
        for (int i = 0; i < charactersByArticle.size(); i++) {
            ...
            while (textIter.hasNext()) {
                TextPosition startPosition = (TextPosition) textIter.next();
                TextPosition endPosition = startPosition;

                PositionWrapper current = new PositionWrapper(startPosition);
                String characterValue = startPosition.getCharacter();
                if (found < matchTo.size()) {
                    while (matchTo.get(found).contains(characterValue) && textIter.hasNext() ) {

                        endPosition = (TextPosition) textIter.next();
                        characterValue = characterValue + endPosition.getCharacter();
                ...
                if (lastPosition != null && ((startPosition.getFont() != lastPosition.getTextPosition().getFont())
                        || (startPosition.getFontSize() != lastPosition.getTextPosition().getFontSize()))) {
                ...
                if (getSortByPosition()) {
                    positionX = startPosition.getXDirAdj();
                    positionY = startPosition.getYDirAdj();
                    positionWidth = startPosition.getWidthDirAdj();
                    positionHeight = startPosition.getHeightDir();
                } else {
                    positionX = startPosition.getX();
                    positionY = startPosition.getY();
                    positionWidth = startPosition.getWidth();
                    positionHeight = startPosition.getHeight();
                }

                //The current amount of characters in a word
                int wordCharCount = startPosition.getIndividualWidths().length;

                /* Estimate the expected width of the space based on the
                 * space character with some margin. */
                float wordSpacing = startPosition.getWidthOfSpace();
                float deltaSpace = 0;
                ...
                int wordCount = characterValue.length();
                try {
                    if (wordCount > matchTo.get(found).length() * (0.75)) {
                        System.out.println("Word count " + wordCount + " " + characterValue + " str : " + searchedWords.get(found));
                        found++;

                        PDFont font = startPosition.getFont();

                        PDPage docPage = (PDPage) document.getDocumentCatalog().getAllPages().get(currentPageNo - 1);
                        List annots = docPage.getAnnotations();
                        ...
                        float ph = 0;
                        if (docPage.findMediaBox() != null) {
                            ph = docPage.findMediaBox().getUpperRightY();
                        } else if (docPage.findCropBox() != null) {
                            ph = docPage.findCropBox().getUpperRightY();
                        } else {
                            ph = 842;
                        }

                        float textWidth = characterValue.length() * averageCharWidth * (float) 1.25;
                        PDRectangle position = new PDRectangle();

                        position.setLowerLeftX(positionX);
                        position.setLowerLeftY(ph - positionY);
                        position.setUpperRightX(positionX + textWidth);
                        position.setUpperRightY(ph - positionY + positionHeight);
...
//                        position.setLowerLeftX(positionX);
//                        position.setLowerLeftY(ph - positionY);
//                        position.setUpperRightX(endPosition.getX());
//                        position.setUpperRightY(ph - endPosition.getY());
//
//                        markup.setRectangle(position);
